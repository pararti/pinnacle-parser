...
type ParticipantRecord struct {
	...
	Team      models.RecordID `json:"team,omitempty"` // Reference to the team
...
// TeamRecord represents a team record in SurrealDB with ID
type TeamRecord struct {
	ID   models.RecordID `json:"id,omitempty"`
	Name string          `json:"name"`
}

// TeamData represents team data without ID for creating new records
...
func (s *SurrealDBClient) FindOrCreateTeam(name string) (*TeamRecord, error) {
	if name == "" {
		return nil, fmt.Errorf("cannot find or create team with empty name")
	}

	ctx, cancel := context.WithTimeout(s.ctx, 10*time.Second)
	defer cancel()

	// First, try to find a team with the given name using a more direct query
	query := "SELECT * FROM teams WHERE name = $name"
	var queryResults []QueryResult[[]TeamRecord]
	err := s.db.WithContext(ctx).Send(&queryResults, "query", query, map[string]interface{}{
		"name": name,
	})
	if err != nil {
		s.logger.Info(fmt.Sprintf("Error querying team: %v", err))
		return nil, fmt.Errorf("failed to query team: %w", err)
	}

	// Check if we found any results
	if len(queryResults) > 0 && len(queryResults[0].Result) > 0 {
		// We found an existing team
		team := &queryResults[0].Result[0]
		s.logger.Info(fmt.Sprintf("Found existing team: Name=%s, ID=%v", name, team.ID))
		return team, nil
	}

	// If not found, create a new team
	s.logger.Info(fmt.Sprintf("Team not found, creating new team: Name=%s", name))

	// Table name for teams
	teamsTable := models.Table("teams")
	...
	// Use Create to let SurrealDB generate a random ID
	createdTeam, err := surrealdb.Create[TeamRecord](s.db.WithContext(ctx), teamsTable, teamData)
	if err != nil {
		s.logger.Info(fmt.Sprintf("Error creating team: %v", err))
		return nil, fmt.Errorf("failed to create team: %w", err)
	}

	s.logger.Info(fmt.Sprintf("Successfully created team: Name=%s, ID=%v", name, createdTeam.ID))
...
func (s *SurrealDBClient) StoreParticipants(participants []*parsed.Participant) ([]*ParticipantRecord, error) {
	...
	for _, participant := range participants {
		...
		// Find or create the team for this participant
		team, err := s.FindOrCreateTeam(participant.Name)
		if err != nil {
			s.logger.Info(fmt.Sprintf("Error finding or creating team: %v", err))
			return nil, fmt.Errorf("failed to find or create team: %w", err)
		}
		...
		participantData := ParticipantData{
			Team:      team.ID, // Reference the team by its ID
		...
		if createdRecord != nil {
			...
			s.logger.Info(fmt.Sprintf("Successfully stored participant: Team=%s, Alignment=%s, ID=%v",
				participant.Name, participant.Alignment, createdRecord.ID))
		}
	}
...
func (s *SurrealDBClient) StoreMatch(match *parsed.Match) error {
	...
	for i, participant := range participants {
		// Create relationship between match and participant
		...
		// Also create a relationship between match and team for easier querying
		teamRelation := &surrealdb.Relationship{
			In:       participant.Team,         // The target node (team)
			Out:      matchRecordID,            // The source node (match)
			Relation: models.Table("has_team"), // The edge type
			Data: map[string]any{
				"created_at": customStartTime,
				"order":      i, // Store the order of teams (e.g., home/away)
				"alignment":  match.Participants[i].Alignment,
			},
		}

		// Create the relationship using InsertRelation
		err = surrealdb.InsertRelation(s.db.WithContext(ctx), teamRelation)
		if err != nil {
			s.logger.Info(fmt.Sprintf("Error creating relationship between match and team: %v", err))
			return fmt.Errorf("failed to create match-team relationship: %w", err)
		}
	}
